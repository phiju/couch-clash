  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className={`font-black ${compact ? "text-3xl" : "text-4xl lg:text-6xl"}`}>Was spielen wir?</h2>
            ...
            >
              <label className="flex cursor-pointer items-start gap-3">
                <input
                  ...
                  className="mt-1.5 size-7 shrink-0 accent-[var(--color-spot)]"
                  aria-label={meta.name}
                />
                <span className={compact ? "text-4xl" : "text-6xl"}>{meta.emoji}</span>
                <span className="flex flex-1 flex-col gap-1">
                  <span className={`flex flex-wrap items-center gap-2 font-black ${compact ? "text-2xl" : "text-3xl lg:text-4xl"}`}>
                    {meta.name}
                    {position >= 0 && (
                      <span className="rounded-full bg-spot px-2.5 py-0.5 text-base text-stage">{position + 1}.</span>
                    )}
                  </span>
                  {!compact && <span className="text-lg text-white/70 lg:text-xl">{meta.description}</span>}
                  <span className="flex flex-wrap gap-2 pt-1 text-sm font-bold text-white/60">
                    <span className="rounded-full bg-white/10 px-2 py-0.5">ab {meta.ageRating}</span>
                    {!compact &&
                      meta.tags.map((t) => (
...
function ScoringEditor({
  ...
  return (
    <details className="rounded-2xl bg-black/20 p-3">
      <summary className="cursor-pointer text-base font-bold text-white/80">Punkte-Einstellungen</summary>
      <div className="mt-3 grid gap-3 text-base">
        {fields.has("basePoints") && (
          <label className="flex items-center justify-between gap-4">
            ...
            <input
              ...
              className="w-24 rounded-xl bg-white px-3 py-1 text-right font-bold text-stage"
            />
          </label>
        )}
        ...
        <button type="button" onClick={onReset} className="self-start text-sm text-white/60 underline">
