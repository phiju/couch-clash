...
export function HostPlay({ room, send }: { room: PublicRoomState; send: Send }) {
  ...
  const skipLabel = moduleState.step === "question" ? "Auflösen ⏭" : "Weiter ⏭";
  return (
  ...
  return (
    <Screen className="max-w-[1400px] gap-8 lg:py-10">
      ...
      <ol className="grid w-full gap-4">
        {ranked.map(({ player, rank, score, gain }, i) => (
          <li
            key={player.id}
            className={`flex animate-pop items-center gap-6 rounded-3xl px-6 py-4 ${rank === 1 ? "bg-spot text-stage" : "bg-white/10"}`}
            style={{ animationDelay: `${(ranked.length - i) * 120}ms`, animationFillMode: "backwards" }}
          >
            <span className="w-14 text-center text-5xl font-black">{rank}.</span>
            <AvatarBadge avatar={player.avatar} size="sm" dimmed={!player.connected} />
            <span className="flex-1 truncate text-4xl font-black">{player.name}</span>
            {gain > 0 && (
              <span className={`text-2xl font-bold ${rank === 1 ? "text-stage/70" : "text-cool"}`}>+{gain}</span>
            )}
            <span className="w-40 text-right text-5xl font-black tabular-nums">{score}</span>
          </li>
        ))}
      </ol>
...
export function HostFinale({ room, send }: { room: PublicRoomState; send: Send }) {
  const game = room.game;
  if (!game) return <Screen />;
  const ranked = rankPlayers(room.players, game.scores);
  ...
  return (
    <Screen className="max-w-[1400px] gap-8 lg:py-10">
      <Confetti />
      <h2 className="text-center text-6xl font-black lg:text-8xl">
        🏆 {winners.map((w) => w.player.name).join(" & ")} {winners.length > 1 ? "gewinnen" : "gewinnt"}!
      </h2>
      <div className="flex w-full flex-1 items-end justify-center gap-6">
        {podium.map((entry, i) =>
          entry ? (
            <div key={entry.player.id} className="flex w-56 flex-col items-center gap-3">
              <AvatarBadge avatar={entry.player.avatar} size={entry.rank === 1 ? "lg" : "md"} className={entry.rank === 1 ? "animate-float" : ""} />
              <span className="max-w-full truncate text-3xl font-black">{entry.player.name}</span>
              <span className="text-2xl font-bold text-white/80">{entry.score} Punkte</span>
              <div
                className={`flex w-full items-start justify-center rounded-t-3xl pt-4 text-6xl font-black ${heights[i]} ${
                  entry.rank === 1 ? "bg-spot text-stage" : "bg-white/15"
                }`}
              >
                {entry.rank}
              </div>
      ...
      {ranked.length > 3 && (
        <ol className="flex flex-wrap justify-center gap-4 text-xl">
          {ranked.slice(3).map(({ player, rank, score }) => (
            <li key={player.id} className="rounded-full bg-white/10 px-4 py-2">
