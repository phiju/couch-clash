...
- **Motifs:** `packages/content/data/pixelpanik/motive.json` (169, zod schema `PixelpanikMotifSchema`). `modes`: `kinder` → Kids, `erwachsene` → Familie, `party` → Party; Pixelpanik is **mode-neutral** (`CategoryMeta.modeNeutral`): no party share per round and the motifs' mode tags are ignored – Party plays every motif, Familie / Kids still never see party-only motifs (global mode filter), Kids only motifs with `kids_choices`. The difficulty mix (leicht / gemischt / schwer) weights the pick. Never the same motif twice per room; topics are mixed (never two in a row when avoidable). **Motifs without `image` are never played** – until the image script ran, the game is hidden in the settings.
...
- **Flow on the show side:** after the last normal round there are no standings – straight to the finale's intro (`applyModuleUpdate` in `game-flow.ts`). Intro = the rules (`introMs`), then the **start sequence** (step `launch`): everyone stands low over the slime with the main-game points, after a 1 s pause the moderator opens the finale (FINALE_STARTED pool, event `LAUNCH`), when his line has ended all elevators ride up at the same speed to their start score (the leader rides `riseMs` = 3 s, the others stop earlier), points counting in step with the car (`launchFrame`); rise sound with the ride, a quieter clack (`survival-elevator-jolt` at 0.5) at every stop; then the first question right away. The last elimination's reveal only lasts until the car is under the slime; the winner's platform rides up (1.5 s), the moderator's WINNER line, then a TRANSITION_TO_CEREMONY line, and when that has ended the ceremony follows. The flow waits for the moderator by **cues**: the host screen reports every line's end, `SurvivalVoice` knows which line the finale waits for and the room moves its timer up (`survival-room.ts`, `survivalCueAt`); without voice / a line it moves on at once (winner: at least `winnerMinMs`), and each step has a safety time (`launchLineMaxMs`, `winnerMs`).
- **Ceremony:** a podium (2 · 1 · 3, places by the finale's elimination order; two players → step 3 empty) with the standing figures: the winner holds the trophy (figure `pokal`, made in the background for both of the last two at FINAL_TWO from their standing standard figure, prompt `trophy` in `figure-prompts.json`, cost logged like the others; fallback: the cheering figure with a drawn cup), places 2 and 3 in their standard figure with slime drips (`components/host/survival-podium.tsx`).
- **TV** (`apps/web/src/games/survival/`): candidates on elevators over a slime pool (CSS/SVG). The slime reaches the screen's bottom edge and lies over the lanes; every lane is clipped at the slime surface (`clip-path`) and an eliminated car is hidden at the end of its animation – nothing ever shows under the slime. Preview of stage and podium with 2–10 made-up players: `/dev/survival?n=6&scene=elim` (scenes `launch`, `elim`, `winner`, `podium`; `?fig=1` with standing figures; in `next dev` right away, otherwise once with `?dev=1`). The elevator height is a separate visual position (`visualHeight`, stretched near the slime) – never the score itself. Live descent, danger states (WARNING → CRITICAL → ELIMINATION_IMMINENT), splash + „ELIMINIERT“, final two, winner. Sound/effect hooks: DOM event `couchclash:survival` for every game event plus `DECAY_TICK`.
- **Sounds** (`apps/web/public/audio/survival-*`, mapping in `docs/survival-sounds.md`): ids are the file names; one-shots MP3, the three loops seamless WAV played by the Web Audio API with `loop = true` over the whole file. `apps/web/src/games/survival/sounds.ts` maps the hook events to sounds (FINALE_STARTED → intro, FAST_CORRECT → bonus, WRONG_ANSWER / TIMEOUT → wrong + elevator jolt, DECAY_TICK → tick per −10, ELIMINATED → splash on the impact, FINAL_TWO, WINNER) and the live danger to the loops (AMBIENCE: slime bubbling always, threat loop from CRITICAL by danger, warning lamp at ELIMINATION_IMMINENT). Levels for every sound in one place: `SOUND_LEVELS` (+ `SOUND_CUE_VOLUMES`) in `apps/web/src/lib/audio/scenes.ts` (the bubbling loop at 0.75); on their own bus, lowered while the host speaks; a missing file never stops the game. Alternatives in `public/audio/alternativen/`.
